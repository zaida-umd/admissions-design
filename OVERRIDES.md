...
using the plain rich-text table. **That page is on the `interior-pages-handoff`
branch, not `main`** (handed to another author 2026-09-14, pull request pending),
so `main` currently has no page exercising these three rules — they are recorded
here so the next rich-text table gets them. Two other tables exist and are **deliberately
